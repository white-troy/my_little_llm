...
class Model_args:
    ...
    vocab_size: int = 100256 # GPT-2 vocab_size of 50257, padded up to nearest multiple of 64 for efficiency 使用c100k_base
    n_layer: int = 12
    n_head: int = 12
    n_embed: int = 768
...
class MLP_GPT(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.c_fc    = nn.Linear(args.n_embed, 4 * args.n_embed, bias=args.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(4 * args.n_embed, args.n_embed, bias=args.bias)
        self.dropout = nn.Dropout(args.dropout)
...
class Block(nn.Module):
    ...
    def __init__(self, args):
        super().__init__()
        self.norm1 = RMS_Norm(args.n_embed)
        self.attn = flash_att(args)
        self.norm2 = RMS_Norm(args.n_embed)
        self.mlp = MLP(args)

    def forward(self,x):
        # 使用pre norm
        x = x + self.attn(self.norm1(x))# residual
        return x + self.mlp(self.norm2(x)) # 残差链接
